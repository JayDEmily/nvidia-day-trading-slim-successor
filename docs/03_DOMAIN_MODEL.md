...
`TerminalRiskApplicationSurface` freezes one additive seam between the overlay-evaluation decision and the later terminal-risk application step. It preserves the raw overlay decision, the final terminal decision, the posture permission state used during the final application, and one bounded overlap-class ledger so later review and replay consumers can inspect why the terminal action stayed aligned with the overlay, derisked further, or escalated to a posture block.

Gate 147 keeps `ExecutionExpressionOutput.final_risk_join` and the existing final-risk stage packet intact for compatibility. The new seam is carried additively inside `StageLocalHandoffSurface` through `overlay_risk_decision` and `terminal_risk_application`; downstream review/trace consumers are migrated in Gate 148 rather than here.

Gate 148 keeps the existing nested review packet and final-risk compatibility fields intact, but review and bounded-trace consumers now also consume the preserved seam surfaces directly. `ReviewExplanationOutput.review_packet` exposes additive top-level `admissibility_surface`, `candidate_ownership`, `overlay_risk_decision`, and `terminal_risk_application` entries, while `BoundedTraceRunResult` carries the same admitted seam models so legacy expectation tests can compare preserved seam truth against `final_risk_join` without mining only the older nested packets.
...
### 5f. Parallel-risk lane objects

Typed contracts for the co-resident independent parallel risk lane.

The lane is co-resident with the serial seven-stage desk grammar without becoming an eighth stage. It may read approved invariants from session start and may add lawful reads of serial-stage outputs once those outputs exist.
...
Ownership:
- binding schema authority in `src/nvda_desk/schemas/parallel_risk.py`
- runtime integration in `src/nvda_desk/services/parallel_risk_lane.py` and `src/nvda_desk/services/cognition_runtime.py`
- runtime carriage in `src/nvda_desk/schemas/cognition.py`
