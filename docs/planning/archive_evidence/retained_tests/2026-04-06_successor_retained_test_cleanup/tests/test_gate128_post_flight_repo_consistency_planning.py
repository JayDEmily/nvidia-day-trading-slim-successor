...
EXECUTION_LOG = (
    REPO_ROOT / "docs/planning/2026-03-31_POST_FLIGHT_REPO_CONSISTENCY_EXECUTION_LOG_v1.md"
)
CHECKLIST = (
    REPO_ROOT
    / "docs/planning/2026-03-31_POST_FLIGHT_REPO_CONSISTENCY_DOCUMENT_TOUCH_CHECKLIST_v1.md"
)
...
def test_post_flight_repo_consistency_pack_is_active() -> None:
    ...
    assert (
        ("2026-03-31_POST_FLIGHT_REPO_CONSISTENCY_LEAVES_v1.json" in plans)
    ...
    assert (
        ("2026-03-31_POST_FLIGHT_REPO_CONSISTENCY_EXECUTION_LOG_v1.md" in plans)
    ...
    assert (
        ("2026-03-31_POST_FLIGHT_REPO_CONSISTENCY_DOCUMENT_TOUCH_CHECKLIST_v1.md" in plans)
        or (
    ...
    assert (
        ...
        in gates
        or "Status: active post-flight repo consistency pack; Gate 128 complete on `main`, Gate 129 active, Gates 130-131 planned"
        in gates
        or "Status: active post-flight repo consistency pack; Gates 128-129 complete on `main`, Gate 130 active, Gate 131 planned"
        in gates
        or "Status: active post-flight repo consistency pack; Gates 128-130 complete on `main`, Gate 131 active"
        in gates
        or "Status: closed post-flight repo consistency pack on `main`; Gates 128-131 complete, no active gate"
        in gates
    )
    assert leaves["execution_status"] in {
        ...
        "gate_130_complete_gate_131_active_on_main",
        "post_flight_repo_consistency_pack_closed_through_gate_131_on_main",
    }
...
def test_pack_freezes_verified_failure_inventory_and_scope_boundaries() -> None:
    ...
    assert (
        leaves["verified_postflight_inventory"]["observed_result"]
        == "429 passed, 14 failed in 33.17s"
    )
    assert len(leaves["verified_postflight_inventory"]["failing_tests"]) == 14
    assert (
        leaves["global_rules"][
            "router_assertions_must_track_modern_pack_state_not_obsolete_literal_plans_text"
        ]
        is True
    )
    assert (
        leaves["global_rules"][
            "runtime_expectation_updates_must_be_backed_by_current_observed_outputs"
        ]
        is True
    )
    assert (
        leaves["global_rules"][
            "no_new_runtime_architecture_or_coefficient_scope_expansion_in_this_pack"
        ]
        is True
    )
