...
def test_gate68_docs_freeze_bounded_precursor_scope() -> None:
    ...
    assert (
        "**Precursor context must come from the bounded Asia/ex-US universe; no casual source creep.**"
        in guardrails
    )
...
def test_gate68_schema_surface_exposes_allowed_fields_and_exclusions() -> None:
    ...
    assert [item.value for item in PrecursorSourceClass] == [
        "cash_equity_index",
        "index_futures",
    ]
    ...
    contract = PrecursorVenueContract(
        ...
        allowed_raw_fields=[
            RawPrecursorField.SESSION_RETURN_PCT,
            RawPrecursorField.FUTURES_BASIS_PCT,
        ],
        ...
        rationale=[
            "Mainland index futures help explain carry-sensitive precursor pressure before the US open."
        ],
    )
    ...
    review = ReviewExplanationOutput(
        summary="precursor universe bounded",
        review_packet={},
