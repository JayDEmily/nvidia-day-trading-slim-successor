...
def test_options_flow_context_detects_negative_gamma_flush_and_vvix_dislocation() -> None:
    ...
    result = service.evaluate(
        OptionsFlowContextInput(
            ...
            front_realised_vol=0.595,
            next_realised_vol=0.58,
            vix_level=29.0,
            vvix_level=118.0,
...
def test_options_flow_context_extracts_live_pin_cluster_and_pin_progression() -> None:
    ...
    result = service.evaluate(
        OptionsFlowContextInput(
            ...
            tenor_iv_curve=[
                TenorCurvePoint(tenor_dte=4, atm_iv=0.58),
                TenorCurvePoint(tenor_dte=11, atm_iv=0.584),
                TenorCurvePoint(tenor_dte=25, atm_iv=0.591),
            ],
...
def test_options_flow_context_marks_compression_breakout_and_iv_curve_balance() -> None:
    ...
    result = service.evaluate(
        OptionsFlowContextInput(
            ...
            front_realised_vol=0.60,
            next_realised_vol=0.61,
            vix_level=18.0,
            vvix_level=82.0,
            ...
            tenor_iv_curve=[
                TenorCurvePoint(tenor_dte=7, atm_iv=0.58),
                TenorCurvePoint(tenor_dte=14, atm_iv=0.585),
                TenorCurvePoint(tenor_dte=28, atm_iv=0.59),
            ],
        )
    )
