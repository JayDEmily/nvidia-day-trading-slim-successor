...
def test_playbook_registry_yaml_round_trips_through_typed_document() -> None:
    ...
    assert (
        restored.execution_template_index()["pin_reversion_exec"].entry_style == "pin_fade_scaler"
    )
...
def test_playbook_registry_matches_live_backfill_fixture_exactly() -> None:
    ...
    assert [playbook.playbook_id for playbook in document.ordered_playbooks()] == fixture[
        "ordered_playbooks"
    ]
    assert [variant.setup_variant_id for variant in document.ordered_setup_variants()] == fixture[
        "ordered_setup_variants"
    ]
