...
def test_gate76_status_closeout_and_leaf_progress_are_recorded() -> None:
    ...
    assert "## Gate 76 — Precursor runtime binding and review exposure\n\nStatus: complete on `main`" in gates_text
    ...
    assert leaves["completed_gate_ids"][:18] == [
        ...
        "Gate 67", "Gate 68", "Gate 69", "Gate 70", "Gate 71", "Gate 72", "Gate 73", "Gate 74", "Gate 75", "Gate 76",
    ]
    assert int(leaves["active_gate"].split()[1]) >= 77
...
def test_gate76_docs_freeze_precursor_runtime_binding_law() -> None:
    ...
    assert "precursor runtime truth must travel in one additive typed packet shared by runtime ingress and review" in normative
    assert "legacy consumers may ignore the richer packet, but they must not rewrite or shadow it" in normative

    assert "## Gate 76 precursor runtime-binding authority" in operating_model
    assert "`PreparedRuntimeSnapshot.precursor_runtime_packet` and `TemporalContextInput.precursor_runtime_packet` carry the same typed precursor truth" in operating_model
...
def test_gate76_precursor_packet_flows_through_chain_to_cognition_and_review() -> None:
    ...
    result = runtime.run(
        temporal_input=fixture.temporal_input.model_copy(update={"precursor_runtime_packet": packet}),
    ...
    assert result.review.precursor_runtime_binding.contradiction_class is PrecursorContradictionClass.DIRECTIONAL_SPLIT
    assert result.review.review_packet["precursor_runtime_binding"] == ReviewPacketService.render_precursor_runtime_binding(packet)
