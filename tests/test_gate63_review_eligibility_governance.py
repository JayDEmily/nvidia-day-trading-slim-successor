...
def test_gate63_docs_freeze_evidence_floors_and_no_change_review() -> None:
    ...
    assert "review eligibility requires a governed evidence block with explicit minimum floors" in normative
    assert "review may conclude `review_not_eligible` or `review_no_change`" in normative
    assert "bounded adjustment request, candidate replacement request, research reset, and missing-module suspicion" in normative

    assert "## Gate 63 review-eligibility authority" in operating_model
    assert "review triggers are bounded to corridor breaches, persistence failures, or coverage collapse" in operating_model
    ...
    assert "**Review eligibility requires governed evidence floors and may conclude no change.**" in guardrails


def test_gate63_schema_surface_exposes_evidence_triggers_and_outcomes() -> None:
    assert [item.value for item in ReviewSurfaceClass] == ["coefficient_group", "policy_surface"]
    ...
    eligibility = ReviewEligibilitySurface(
        ...
        trigger_classes=[ReviewTriggerClass.MATERIAL_CORRIDOR_BREACH, ReviewTriggerClass.PERSISTENCE_FAILURE],
        eligible=True,
        governed_outcome=ReviewOutcome.BOUNDED_ADJUSTMENT_REQUEST,
        change_budget=ReviewChangeBudget.BOUNDED_SINGLE_SURFACE,
    )
    review = ReviewExplanationOutput(summary="review needed", review_packet={}, review_eligibility=eligibility)
