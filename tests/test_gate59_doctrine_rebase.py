...
LEGACY_GATES_V45 = REPO_ROOT / "docs/legacy/2026-03-27_COGNITIVE_WORKFLOW_MODIFICATION_GATES_v4_5.md"
LEGACY_LEAVES_V45 = REPO_ROOT / "docs/legacy/2026-03-27_COGNITIVE_WORKFLOW_MODIFICATION_LEAVES_v4_5.json"
...
def test_v6_pair_is_the_only_active_successor_pointer() -> None:
    plans = PLANS.read_text()
    gate_map = GATE_MAP.read_text()

    assert 'The active successor modification pair from Gate 59 onward is:' in plans
    ...
    assert 'Current active gate: **Gate 60 in the V6 successor pack**.' in gate_map
...
def test_v6_gates_doc_is_self_contained_and_governing_inputs_do_not_depend_on_missing_drafts() -> None:
    ...
    governing_inputs = _section(gates_text, '## Governing inputs', '## Historical salvage consulted for Gate 59 closeout')
    ...
    assert 'Status: complete on `main`' in _section(gates_text, '## Gate 59 — Doctrine rebase and tranche reset', '## Gate 60 — State-policy vocabulary and coefficient ontology')


def test_gate59_doctrine_statements_land_across_normative_operating_and_guardrail_surfaces() -> None:
    normative = NORMATIVE.read_text()
    operating_model = OPERATING_MODEL.read_text()
    guardrails = GUARDRAILS.read_text()

    assert 'stable cognition grammar and only allows bounded, approved change in operating posture' in normative
    assert 'live paper is the falsification and promotion surface for locked candidates' in normative
    ...
    assert 'Live paper is the falsification/promotion surface for locked candidates' in operating_model
    assert 'Review may end in `no_change`' in operating_model

    assert '**Stable cognition grammar is preserved unless a deliberate normative change proves otherwise.**' in guardrails
    assert '**Live paper is falsification/promotion only, never in-place coefficient discovery.**' in guardrails
...
def test_v6_leaves_ledger_marks_gate59_complete_and_gate60_next() -> None:
    ...
    assert leaves['execution_status'] == 'gate_59_complete_on_main_successor_pack_active_from_gate_60'
    assert leaves['completed_gate_ids'] == ['Gate 59']
    assert leaves['active_gate'] == 'Gate 60'
...
def test_v6_gate_order_and_leaves_order_are_monotonic() -> None:
    ...
    gate_numbers_from_md = [int(match) for match in re.findall(r'^## Gate (\d+) —', gates_text, flags=re.MULTILINE)]
