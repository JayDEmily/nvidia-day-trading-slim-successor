...
def test_gate72_docs_freeze_provenance_and_outage_law() -> None:
    ...
    assert (
        "**Event sources must normalise through the frozen provenance contract; stale, conflicting, or degraded source state must stay visible.**"
        in guardrails
    )
...
def test_gate72_schema_surface_and_service_normalise_supported_sources() -> None:
    ...
    assert [item.value for item in SupportedEventSource] == [
        ...
        "policy_release_calendar",
        "internal_event_ledger",
        "options_expiry_ledger",
    ]
    assert [item.value for item in EventFreshnessState] == [
        "current",
        "stale",
        "deferred",
    ]
    assert [item.value for item in EventConfidenceTier] == [
        "authoritative",
        "corroborated",
        "provisional",
        "degraded",
    ]
    assert [item.value for item in SourceConflictDisposition] == [
        "authoritative_source_wins",
        "latest_corroborated_wins",
        "keep_conflict_visible",
    ]
    assert [item.value for item in SourceOutagePolicy] == [
        "use_last_verified_with_flag",
        "degrade_to_unknown",
        "drop_source_and_block_unsupported",
    ]
    ...
    assert (
        authority.authority.source_inventory[0].source
        is SupportedEventSource.NASDAQ_TRADER
    )
