...
def _context_from_fixture(*, stressed: bool = False) -> PostureEnricherContext:
    ...
    return PostureEnricherContext(
        ...
        macro_signal_score=cast(
            MacroSignalScoreContractOutput, scanner_outputs["macro_signal_score"]
        ),
        ...
        vix_spread_detector=cast(
            VixSpreadDetectorContractOutput, scanner_outputs["vix_spread_detector"]
        ),
...
def test_posture_enrichers_keep_fill_bias_and_tail_hedge_advisory_with_explicit_provenance() -> (
    None
):
    ...
    assert {
        fence.dependency
        for fence in fill_bias.dependency_fences
    ...
    assert tail_hedge.upstream_contract_slugs == [
        "macro_signal_score",
        "vix_spread_detector",
        "engine_score",
    ]
    ...
    assert {
        fence.dependency
        for fence in tail_hedge.dependency_fences
        if fence.status.value == "satisfied"
    } == {
...
def test_posture_enrichers_reflect_stressed_conditions_and_keep_obv_flow_fenced() -> None:
    ...
    assert {
        fence.dependency
        for fence in obv_confirmation.dependency_fences
