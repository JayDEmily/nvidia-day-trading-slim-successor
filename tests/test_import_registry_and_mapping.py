...
def test_gate_b_seed_files_exist_and_cover_the_same_universe() -> None:
    ...
    assert {item.canonical_id for item in registry_seed} == {item.canonical_id for item in mapping_seed}
...
def test_refresh_rebuilds_enriched_surfaces_from_seed_inputs() -> None:
    ...
    assert rebuilt_first.implementation_readiness == ImplementationReadiness.READY_FOR_CONTRACT_IMPORT
...
def test_canonical_registry_summary_matches_preservation_split() -> None:
    ...
    assert summary["item_class_counts"] == {"cognition_fragment": 26, "feature": 32, "module": 71}
...
def test_registry_rows_expose_priority_readiness_and_affinity_fields() -> None:
    ...
    service = CanonicalImportRegistryService(Path.cwd())
    registry = service.load_registry()
    ladder_constructor = next(item for item in registry if item.canonical_slug == "ladder_constructor")
    options_fragment = next(item for item in registry if item.canonical_slug == "options_flow_context_first_class")
    ...
    assert ladder_constructor.implementation_readiness == ImplementationReadiness.READY_FOR_CONTRACT_IMPORT
...
def test_mapping_rows_expose_contract_status_priority_and_affinity_fields() -> None:
    ...
    options_fragment = next(item for item in mapping if item.canonical_slug == "options_flow_context_first_class")
    ladder_constructor = next(item for item in mapping if item.canonical_slug == "ladder_constructor")
    ...
    assert macro_shock.inventory_affinity is True
    assert options_fragment.runtime_contract_status == RuntimeContractStatus.EVIDENCE_ONLY_NO_CONTRACT
    assert options_fragment.options_affinity is True
...
def test_gate_b_reports_exist_and_exclude_evidence_only_from_executable_backlog() -> None:
    ...
    assert provenance_report["preservation_only_count"] + provenance_report["implemented_or_translated_count"] == 129
...
def _load_tranche_a_manifest() -> dict[str, Any]:
    manifest_path = Path.cwd() / "docs" / "planning" / "2026-03-24_CONTRACT_IMPORT_TRANCHE_A_MANIFEST.json"
...
def test_tranche_a_manifest_rows_match_backlog_readiness_blockers_and_role_order() -> None:
    ...
    for row in items:
        ...
        assert registry_item.implementation_readiness == ImplementationReadiness.READY_FOR_CONTRACT_IMPORT
    ...
    assert "No fifth playbook or named-playbook expansion inside tranche A." in manifest["scope_boundary"]["stop_rules"]
