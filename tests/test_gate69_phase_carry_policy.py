...
def test_gate69_status_closeout_and_leaf_progress_are_recorded() -> None:
    ...
    assert "## Gate 69 — Phase-of-day and carryover policy matrix\n\nStatus: complete on `main`" in gates_text
    ...
    assert int(leaves["active_gate"].split()[1]) >= 70
...
def test_gate69_docs_freeze_phase_carry_and_no_action_law() -> None:
    ...
    assert "approved day-phase states are `opening_disorder`, `opening_resolution`, `trend_window`, `midday_compression`, `late_session`, `close_auction`, and `post_close`" in normative
    assert "carry-horizon states are `intraday_only`, `overnight_setup`, `weekend_setup`, and `event_carry_setup`" in normative
    assert "midday compression and late-session carry preparation may prefer or require no-action" in normative
    ...
    assert "**Phase/carryover policy may change bounded posture, but it must not force action just because the clock moved.**" in guardrails
...
def test_gate69_schema_surface_extends_state_vector_and_policy_matrix() -> None:
    ...
    assert [item.value for item in PhaseNoActionBias] == ["neutral", "preferred", "required"]
    ...
    record = PhaseCarryPolicyRecord(
        ...
        mutable_surface_targets=[MutableRuntimeSurface.TARGET_FRESH_DEPLOYABLE_PCT, MutableRuntimeSurface.ENTRY_GATE_SCORE_FLOOR],
    ...
    review = ReviewExplanationOutput(summary="phase policy bounded", review_packet={}, phase_carry_policy=surface)
