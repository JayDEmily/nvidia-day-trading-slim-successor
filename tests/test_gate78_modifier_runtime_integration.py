...
def _tightened_precursor_packet() -> PrecursorRuntimePacket:
    return PrecursorRuntimePacket(
        ...
        stitched_order=[PrecursorVenueUniverse.CFFEX_INDEX_FUTURES_COMPLEX, PrecursorVenueUniverse.JPX_CASH_INDEX_COMPLEX],
        active_venues=[PrecursorVenueUniverse.CFFEX_INDEX_FUTURES_COMPLEX, PrecursorVenueUniverse.JPX_CASH_INDEX_COMPLEX],
...
def _unresolved_precursor_packet() -> PrecursorRuntimePacket:
    return PrecursorRuntimePacket(
        ...
        missing_venues=[PrecursorVenueUniverse.JPX_CASH_INDEX_COMPLEX, PrecursorVenueUniverse.HKEX_CASH_INDEX_COMPLEX],
...
def test_gate78_status_closeout_and_leaf_progress_are_recorded() -> None:
    ...
    assert "## Gate 78 — Runtime integration of state-conditioned modifiers\n\nStatus: complete on `main`" in gates_text
    ...
    assert leaves["completed_gate_ids"][:20] == [
        ...
        "Gate 67", "Gate 68", "Gate 69", "Gate 70", "Gate 71", "Gate 72", "Gate 73", "Gate 74", "Gate 75", "Gate 76", "Gate 77", "Gate 78",
    ]
    assert int(leaves["active_gate"].split()[1]) >= 79
...
def test_gate78_docs_freeze_runtime_modifier_integration_law() -> None:
    ...
    assert "runtime must materialise a typed modifier packet before downstream review or replay can talk about effective coefficients honestly" in normative
    assert "blocked or stand-down outcomes must be explicit packet fields, not inferred from missing trades" in normative

    assert "## Gate 78 runtime modifier integration authority" in operating_model
    assert "`PostureRiskOutput`, `ExecutionExpressionOutput`, and `ReviewExplanationOutput` must all be able to see the same Gate 78 modifier packet" in operating_model

    assert "### 5d. Runtime modifier integration objects" in domain_model
    assert "Modifier runtime integration must stay additive, lineaged, and backwards-compatible — no silent rewrites of earlier deterministic consumers." in guardrails
...
def test_gate78_schema_surface_exposes_modifier_runtime_packet() -> None:
    ...
    assert packet.resolved_surfaces[0].target_surface is MutableRuntimeSurface.TARGET_FRESH_DEPLOYABLE_PCT
...
def test_gate78_runtime_applies_deterministic_modifier_caps_and_lineage() -> None:
    ...
    assert "phase_carry:late_session:event_carry_setup" in result.review.review_lineage.modifier_policy_ids
    assert MutableRuntimeSurface.TARGET_FRESH_DEPLOYABLE_PCT.value in result.review.review_lineage.effective_coefficient_targets
...
def test_gate78_runtime_exposes_kill_switch_and_blocked_trade_outcomes() -> None:
    ...
    assert result.review.failure_taxonomy.primary_failure_class is ReviewFailureClass.POSTURE_POLICY_FAILURE
