...
from nvda_desk.services.cognition_runtime_registry import DeskCognitionRuntimeRegistryService
...
def test_runtime_registry_exposes_all_binding_layers_in_order() -> None:
    ...
    options_contract = next(contract for contract in contracts if contract.grammar_role == "options_flow_context")
    ...
    posture_contract = next(contract for contract in contracts if contract.grammar_role == "posture_risk_permission")
    ...
    playbook_contract = next(contract for contract in contracts if contract.grammar_role == "playbook_eligibility")
    assert "rejected_playbook_reasons" in playbook_contract.required_output_fields
    review_contract = next(contract for contract in contracts if contract.grammar_role == "review_explanation")
...
def test_runtime_registry_builds_trace_packets_for_binding_layers() -> None:
    ...
    service = DeskCognitionRuntimeRegistryService()
    packet = service.build_trace_packet(grammar_role="options_flow_context", summary="options state ready")
...
def test_runtime_registry_enforces_docstring_template() -> None:
    ...
    assert template.required_sections == ["Purpose:", "Inputs:", "Outputs:", "Determinism:"]
