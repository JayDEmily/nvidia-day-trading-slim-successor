...
def test_governance_pack_is_closed_honestly_across_the_quartet() -> None:
    ...
    assert ("repo-process governance pack closed through Gate 112 on `main`" in plans) or (
        "2026-03-30_REPO_PROCESS_GOVERNANCE_GATES_v1.md" in plans
    )
    assert any(marker in gate_map for marker in ALLOWED_CURRENT_GATE_MARKERS)
    assert "| Gate 112 | complete on `main` |" in gate_map
    assert (
        "Status: closed repo-process governance pack on `main`; Gates 107-112 complete, no active gate"
        in gates
    )
    assert (
        leaves["execution_status"] == "repo_process_governance_pack_closed_through_gate_112_on_main"
    )
    assert (
        leaves["active_gate"]
...
def test_gate112_closeout_doc_freezes_packaging_intent() -> None:
    ...
    assert (
        "Status: Gate 112 complete on `main`; repo-process governance pack closed honestly"
        in closeout
    )
