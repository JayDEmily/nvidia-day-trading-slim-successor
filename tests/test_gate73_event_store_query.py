...
def test_gate73_status_closeout_and_leaf_progress_are_recorded() -> None:
    ...
    assert "## Gate 73 — Event store and query surfaces\n\nStatus: complete on `main`" in gates_text
    ...
    assert leaves["completed_gate_ids"][:15] == [
        ...
        "Gate 67", "Gate 68", "Gate 69", "Gate 70", "Gate 71", "Gate 72", "Gate 73",
    ]
    assert int(leaves["active_gate"].split()[1]) >= 74
...
def test_gate73_docs_freeze_shared_event_truth_law() -> None:
    ...
    assert "runtime, review, and replay consumers must use the same nearby-event window semantics" in normative
    assert "lineage lookup must return the same normalised provenance keys the runtime saw" in normative

    assert "## Gate 73 shared event-store and query authority" in operating_model
    assert "material-event filters now use explicit materiality floors" in operating_model

    assert "### 4k. Shared event-store and query objects" in domain_model
    assert "**Runtime, review, and replay must query the same shared event truth; no private nearby-event semantics.**" in guardrails


def test_gate73_schema_surface_and_service_share_nearby_truth() -> None:
    assert [item.value for item in ReplayEventConsumerMode] == ["runtime_nearby", "review_lineage", "replay_session"]
    authority = EventStoreAuthorityResponse(
        authority=EventStoreAuthorityPacket(
            default_query_window=EventQueryWindow(lookback_minutes=240, lookahead_minutes=1440),
