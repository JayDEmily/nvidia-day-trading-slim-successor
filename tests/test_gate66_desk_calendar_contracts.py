...
def test_gate66_status_closeout_and_leaf_progress_are_recorded() -> None:
    ...
    assert "## Gate 66 — Session, holiday, and venue-calendar contracts\n\nStatus: complete on `main`" in gates_text
...
def test_gate66_docs_freeze_venue_timezone_and_bridge_law() -> None:
    ...
    assert "the canonical venue set for this tranche is US Nasdaq cash, JPX cash, HKEX cash, SSE/SZSE cash, and CFFEX index futures" in normative
    assert "US early-close and half-day semantics, HK holiday-eve half-day semantics, Japan split-session semantics, and Mainland China bridge/make-up-day semantics" in normative

    assert "## Gate 66 desk-calendar authority" in operating_model
    assert "venue contracts carry timezone authority, session template, closure classes, and bridge rules" in operating_model

    assert "### 4g. Desk-calendar and venue objects" in domain_model
    assert "**Venue calendar truth must come from explicit venue contracts, not one generic market-open flag.**" in guardrails
...
def test_gate66_schema_surface_matches_frozen_calendar_authority() -> None:
    ...
    nasdaq = VenueSessionContract(
        ...
        closure_classes=[CalendarClosureClass.WEEKEND, CalendarClosureClass.FULL_HOLIDAY, CalendarClosureClass.OBSERVED_HOLIDAY, CalendarClosureClass.HALF_DAY],
        bridge_rules=[SessionBridgeRule.US_EARLY_CLOSE],
        notes=["Nasdaq cash follows US full holidays and 1:00pm ET early closes on designated dates."],
    )
    jpx = VenueSessionContract(
        ...
        closure_classes=[CalendarClosureClass.WEEKEND, CalendarClosureClass.FULL_HOLIDAY],
        bridge_rules=[SessionBridgeRule.PRECURSOR_NEXT_US_SESSION_ONLY],
        notes=["JPX cash is split 09:00-11:30 and 12:30-15:30 JST with exchange holidays including Jan 1-3 and Dec 31."],
    )
    hkex = VenueSessionContract(
        ...
        trading_days=["monday", "tuesday", "wednesday", "thursday", "friday"],
        session_segments=["pre_open", "morning_session", "extended_morning_session", "afternoon_session", "closing_auction"],
        closure_classes=[CalendarClosureClass.WEEKEND, CalendarClosureClass.FULL_HOLIDAY, CalendarClosureClass.HOLIDAY_EVE_HALF_DAY],
        bridge_rules=[SessionBridgeRule.HK_HOLIDAY_EVE_HALF_DAY, SessionBridgeRule.PRECURSOR_NEXT_US_SESSION_ONLY],
    ...
    china = VenueSessionContract(
        ...
        trading_days=["monday", "tuesday", "wednesday", "thursday", "friday"],
        session_segments=["opening_call_auction", "morning_session", "lunch_break", "afternoon_session", "closing_call_auction"],
        closure_classes=[CalendarClosureClass.WEEKEND, CalendarClosureClass.FULL_HOLIDAY, CalendarClosureClass.BRIDGE_HOLIDAY, CalendarClosureClass.MAKEUP_WORKING_DAY],
        bridge_rules=[SessionBridgeRule.CHINA_WEEKEND_MAKEUP_WORKDAY, SessionBridgeRule.PRECURSOR_NEXT_US_SESSION_ONLY],
        notes=["Mainland exchanges use explicit holiday calendars with occasional weekend make-up working days and no ordinary cash-market half days."],
    )
    authority = DeskCalendarAuthorityResponse(authority=DeskCalendarAuthorityPacket(
        venues=[nasdaq, jpx, hkex, china],
        closure_classes=list(CalendarClosureClass),
        bridge_rules=list(SessionBridgeRule),
        expiry_interaction_notes=["Expiry on a shortened session must stay explicit rather than inferred by downstream policy."],
    ))
    interaction = CalendarAwareEventContract(
        ...
        notes=["Friday monthly expiry on a US early close is materially different from a normal full session."],
    )
...
def test_gate66_vocabulary_terms_are_present_and_generated() -> None:
    ...
    assert generated["desk_calendar_contract"].maps_to_contract == "nvda_desk.schemas.session_clock.DeskCalendarAuthorityPacket"
