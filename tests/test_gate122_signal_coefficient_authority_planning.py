...
CHECKLIST = REPO_ROOT / "docs/planning/2026-03-31_SIGNAL_COEFFICIENT_AUTHORITY_DOCUMENT_TOUCH_CHECKLIST_v1.md"
...
def test_signal_coefficient_authority_pack_is_active() -> None:
    ...
    assert (
        ...
        or "Status: active signal-coefficient authority pack; Gate 122 complete on `main`, Gate 123 active, Gates 124-127 planned" in gates
...
def test_pack_freezes_tranche_one_scope_and_exclusions() -> None:
    ...
    assert "raw Asia, Japan, Europe, commodity, crypto, or single-stock precursor coefficients" in gates
    assert "timing parameters are separated from behavioural thresholds" in gates or "timing parameters" in gates
    assert leaves["global_rules"]["workbook_is_research_authority_not_runtime_authority"] is True
    assert leaves["global_rules"]["starter_bounds_must_be_semantically_bounded_not_search_wide"] is True
    assert leaves["global_rules"]["asia_precursor_proxies_remain_excluded_from_raw_coefficient_authority"] is True
