...
def test_gate31_coverage_is_closed_in_frozen_order_with_honest_composite_boundaries() -> None:
    ...
    options_cluster = cast(OptionsBehaviourClusterContractOutput, outputs["options_behaviour_cluster"])
    execution_context = cast(ExecutionContextScoreContractOutput, outputs["execution_context_score"])
    compression = cast(CompressionRegimeDetectorContractOutput, outputs["compression_regime_detector"])
    ...
    assert compression.upstream_contract_slugs == ["vol_corridor", "execution_context_score", "options_behaviour_cluster"]
...
def test_gate31_stressed_context_keeps_composites_descriptive_rather_than_promotional() -> None:
    ...
    assert compression.compression_state in {"compression_blocked", "compression_watch", "compression_absent"}
