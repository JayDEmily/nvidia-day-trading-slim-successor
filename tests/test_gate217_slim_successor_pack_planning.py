...
def test_gate217_pack_is_open_and_routed_coherently() -> None:
    ...
    assert "- next active gate: `Gate 217`" in plans
    assert (
        "- active pack: slim active-repo cutover and substantive test-audit bootstrap pack "
        f"with Gate 217 active on `{ACTIVE_BRANCH}`"
    ) in plans
    ...
    assert "Version: v1.29" in gate_map
    assert (
        "Current active gate: **Gate 217 in the slim active-repo cutover and substantive test-audit "
        f"bootstrap pack on `{ACTIVE_BRANCH}`**."
    ) in gate_map
    ...
    assert f"Gate 217 | active on `{ACTIVE_BRANCH}`" in gate_map
    ...
    assert "no later pack is active yet" not in gate_map

    assert (
        "Status: active slim-successor planning pack; Gate 217 active on "
        f"`{ACTIVE_BRANCH}`, Gates 218-221 planned."
    ) in gates
    ...
    assert payload["execution_status"] == "gate_217_active_on_work_branch"
    assert payload["active_gate"] == "Gate 217"
    assert payload["completed_gate_ids"] == []
    assert payload["completed_leaf_ids"] == []
    assert len(payload["remaining_leaf_ids"]) == 11
    assert set(payload["completed_leaf_ids"]).isdisjoint(payload["remaining_leaf_ids"])

    assert (
        "Status: active successor execution log for slim active-repo cutover and substantive test-audit "
        f"bootstrap; Gate 217 active on `{ACTIVE_BRANCH}`, Gates 218-221 planned."
    ) in execution_log
    assert (
        "No completed leaf receipts yet. Gate 217 is active on "
        f"`{ACTIVE_BRANCH}`."
    ) in execution_log
