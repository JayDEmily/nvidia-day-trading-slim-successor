...
from nvda_desk.schemas.review import ImportedModuleApprovalState, ImportedModuleMaturityState
...
def test_review_packet_cites_tranche_a_modules_with_honest_maturity_and_no_fake_approval() -> None:
    ...
    assert result.review.review_packet["imported_module_maturity_counts"] == result.review.imported_module_maturity_counts
    assert result.review.imported_module_maturity_counts == {
        "concept_contract_only": 8,
        "implemented_runtime_proxy": 5,
    }
    assert all(citation.approval_state is ImportedModuleApprovalState.NOT_APPROVED for citation in citations)
    ...
    assert citation_index["volume_spike_filter"].maturity_state is ImportedModuleMaturityState.CONCEPT_CONTRACT_ONLY
    assert citation_index["event_flag_capture"].maturity_state is ImportedModuleMaturityState.IMPLEMENTED_RUNTIME_PROXY
    assert any(
...
def test_replay_run_result_carries_tranche_a_citations_without_changing_report_shape() -> None:
    ...
    run = next(run for run in runs if run.coefficient_set_id == "full_stack_base" and run.scenario_id == "trend")
    assert [citation.canonical_slug for citation in run.imported_module_citations] == EXPECTED_TRANCHE_A_ORDER
    ...
    assert all(citation.approval_state is ImportedModuleApprovalState.NOT_APPROVED for citation in run.imported_module_citations)
