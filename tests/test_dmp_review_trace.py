...
def test_review_output_carries_dmp_lineage_for_the_decision_chain() -> None:
    ...
    assert result.stage_packets[-1].trace_references.review_trace_id == f"review-trace::{lineage.review_packet_id}"
    assert result.stage_packets_v2[-1].lineage.review_trace_id == f"review-trace::{lineage.review_packet_id}"
    assert result.stage_packets_v2[-1].packet_id == lineage.review_packet_id


def test_replay_run_result_carries_replay_packet_lineage_without_touching_report_metrics() -> None:
