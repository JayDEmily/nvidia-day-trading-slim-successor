...
def test_gate70_docs_freeze_event_and_options_stress_law() -> None:
    ...
    assert (
        "**Event/options-stress posture must come from the frozen matrix, not improvised event lore or gamma hand-waving.**"
        in guardrails
    )
...
def test_gate70_schema_surface_exposes_bounded_matrix_and_review_hook() -> None:
    ...
    assert [item.value for item in PolicyEffectType] == [
        "suppress",
        "degrade",
        "widen",
        "cap",
    ...
    record = EventOptionsStressPolicyRecord(
        ...
        mutable_surface_targets=[
            MutableRuntimeSurface.MAX_RISK_PER_TRADE,
            MutableRuntimeSurface.HEDGE_REQUIRED,
        ],
    ...
    surface = EventOptionsStressPolicySurface(
        active_states=[
            EventOptionsStressState.EVENT_IMMINENT,
            EventOptionsStressState.NEGATIVE_GAMMA_STRESS,
        ],
        behaviour_class=EventOptionsBehaviourClass.HEDGED_ONLY,
        effect_types=[PolicyEffectType.CAP, PolicyEffectType.HEDGE],
    )
    review = ReviewExplanationOutput(
        summary="event/options matrix bounded",
        review_packet={},
        event_options_stress_policy=surface,
    )

    assert (
        authority.authority.policy_records[0].behaviour_class
        is EventOptionsBehaviourClass.HEDGED_ONLY
    )
