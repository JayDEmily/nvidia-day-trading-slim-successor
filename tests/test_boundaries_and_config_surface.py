...
def test_config_bundle_parses_typed_weights_and_variants() -> None:
    ...
    assert bundle.coefficients_registry.runtime.layer1_signals["S08"].coefficients["vwap_proximity_pct"].value == 1.5
    assert bundle.strategy_variants.variants["conservative"].overrides.weights["S06"] == 0.25
    assert bundle.strategy_variants.variants["conservative"].overrides.coefficients["L3_01"]["score_floor"] == 0.72
...
async def _exercise_boundary_stubs() -> tuple[str, str, int]:
    ...
    ref = await broker.place_order(OrderIntent(symbol="NVDA", side="buy", quantity=1, limit_price=100.0))
    events = [event async for event in broker.stream_order_events()]
    orchestrator = NullOpenAIOrchestrator()
    artifact = await orchestrator.respond(OpenAIResponseRequest(prompt="Summarise risk state", tool_names=["risk_gateway"]))
