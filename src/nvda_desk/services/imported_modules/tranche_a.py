...
class TrancheAUpstreamContractService:
    ...
    def _event_flag_capture(self, context: TrancheAUpstreamContext) -> EventFlagCaptureContractOutput:
        ...
        return EventFlagCaptureContractOutput(
            ...
            contract_notes=["Event feed remains external; this contract only preserves the runtime-facing event flag surface."],
    ...
    def _realized_volatility_engine(self, context: TrancheAUpstreamContext) -> RealizedVolatilityEngineContractOutput:
        ...
        vol_compression = "compressed" if abs((rv_5d or 0.0) - (rv_10d or 0.0)) <= 2.0 else "expanded"
        return RealizedVolatilityEngineContractOutput(
            ...
            dependency_fences=_dependency_fences(
                ["spot_prices"],
                proxied={"spot_prices": "proxied from existing realised-volatility fields already passed into OptionsFlowContextInput"},
            ),
            contract_notes=["Direct spot-price history remains outside the current runtime; the contract uses existing realised-volatility proxies only."],
            rv_5d=rv_5d,
            rv_10d=rv_10d,
    ...
    def _volume_spike_filter(self, context: TrancheAUpstreamContext) -> VolumeSpikeFilterContractOutput:
        return VolumeSpikeFilterContractOutput(
            ...
            contract_notes=["The current runtime has no volume-series surface, so the detector remains an honest contract fence."],
            signal_score=0.0,
            spike_count=None,
            trap_flag=None,
    ...
    def _peer_divergence(self, context: TrancheAUpstreamContext) -> PeerDivergenceContractOutput:
        ...
        return PeerDivergenceContractOutput(
            ...
            contract_notes=["Peer-equity granularity remains fenced; this contract uses current cross-asset proxies only."],
    ...
    def _gamma_pressure(self, context: TrancheAUpstreamContext) -> GammaPressureContractOutput:
        ...
        return GammaPressureContractOutput(
            ...
            contract_notes=["Full chain and metadata remain fenced; the contract relies on the existing gamma-pressure runtime surface."],
    ...
    def _iv_vs_rv_analysis(self, context: TrancheAUpstreamContext) -> IvVsRvAnalysisContractOutput:
        ...
        return IvVsRvAnalysisContractOutput(
            ...
            contract_notes=["The contract uses current ATM-IV and realised-volatility inputs rather than a fresh live chain pull."],
    ...
    def _skew_inflection(self, context: TrancheAUpstreamContext) -> SkewInflectionContractOutput:
        ...
        return SkewInflectionContractOutput(
            ...
            dependency_fences=_dependency_fences(
                ["options_chain"],
                proxied={"options_chain": "proxied from repeated put_call_skew snapshots already carried by OptionsFlowContextInput"},
            ),
            contract_notes=["Skew inflection is reconstructed from existing repeated snapshots; a full live chain still remains out of scope."],
...
class TrancheASelectorContractService:
    ...
    def _signal_conflict_detector(self, context: TrancheASelectorContext) -> SignalConflictDetectorContractOutput:
        ...
        if context.options_flow.gamma_state.value == "destabilising" and context.posture.permission_state.value != "block":
        ...
        return SignalConflictDetectorContractOutput(
            ...
            contract_notes=["Execution decisions remain partially fenced until the selector is bound into a richer decision trace."],
    ...
    def _model_confidence_scorer(self, context: TrancheASelectorContext) -> ModelConfidenceScorerContractOutput:
        ...
        confidence = round(max(0.0, min(1.0, 0.45 + signal_bonus + permission_bonus - gamma_penalty)), 4)
        ...
        return ModelConfidenceScorerContractOutput(
            ...
            contract_notes=["Engine score remains fenced; confidence uses current stage outputs plus explicit conflict state."],
            model_confidence=confidence,
            confidence_band=band,
        )

    def _conviction_tier_allocator(self, context: TrancheASelectorContext) -> ConvictionTierAllocatorContractOutput:
        score = 0.65 if context.posture.permission_state.value == "allow" else 0.45 if context.posture.permission_state.value == "derisk" else 0.1
        ...
        return ConvictionTierAllocatorContractOutput(
            ...
            dependency_fences=_dependency_fences(
                ...
                proxied={"engine_score": "proxied from current permission, volatility, and options cluster surfaces"},
            ),
            contract_notes=["Macro signal score remains fenced; conviction is advisory only inside tranche A."],
    ...
    def _entry_gate(self, context: TrancheASelectorContext) -> EntryGateContractOutput:
        ...
        return EntryGateContractOutput(
            ...
            dependency_fences=_dependency_fences(
                ["engine_score", "macro_signal_score"],
                proxied={"engine_score": "proxied from current posture permission and temporal veto surfaces"},
            ),
    ...
    def _ladder_constructor(self, context: TrancheASelectorContext) -> LadderConstructorContractOutput:
        if context.posture.permission_state.value == "block":
            return LadderConstructorContractOutput(
                ...
                contract_notes=["No ladder is proposed while permission is blocked; metadata dependency remains fenced."],
                expiry=None,
                ladder_span=None,
                ladder_strikes=[],
        ...
        return LadderConstructorContractOutput(
            ...
            contract_notes=["Metadata remains fenced; ladder levels are advisory and derived from the current implied-move envelope."],
            expiry=None,
            ladder_span=span,
    ...
    def _archetype_matcher(self, context: TrancheASelectorContext) -> ArchetypeMatcherContractOutput:
        ...
        return ArchetypeMatcherContractOutput(
            ...
            dependency_fences=_dependency_fences(
                ...
                proxied={"spot_prices": "proxied from current options-flow spot and pattern surfaces"},
            ),
            contract_notes=["Optional context signals remain fenced; this archetype match only reflects the current deterministic runtime surface."],
