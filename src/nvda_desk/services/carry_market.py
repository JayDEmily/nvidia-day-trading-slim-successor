    ...
    def evaluate_from_market(self, payload: OvernightCarryMarketInput) -> OvernightCarryMarketOutput:
        ...
        if not bars:
            derived = CarryDerivedContext(
                ...
                close_phase=self._classifier.classify(payload.evaluation_ts).phase,
            ...
            return OvernightCarryMarketOutput(**output.model_dump(), derived_context=derived)
        ...
        close_phase = self._classifier.classify(last_bar.ts_utc.astimezone(UTC)).phase
        derived = CarryDerivedContext(
            evaluation_ts=payload.evaluation_ts,
