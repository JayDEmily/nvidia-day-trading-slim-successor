...
class RealDataLoaderService:
    ...
    def build_fixture_pack(self, bundle: RealDataBundle, *, pack_id: str) -> PreparedRuntimeFixturePack:
    ...
    def build_runtime_snapshot_sanity_report(
        ...
        max_bar_age_seconds = max((snapshot.bar_age_seconds for snapshot in prepared_dataset.snapshots), default=0)
        monotonic_snapshot_timestamps = all(
            left.ts <= right.ts
            for left, right in zip(prepared_dataset.snapshots, prepared_dataset.snapshots[1:], strict=False)
        )
        total_bars = len(bundle.bars)
        total_chains = len(bundle.option_chain_snapshots)
        aligned_bar_coverage_pct = round((len(used_bar_ts) / total_bars) * 100.0, 4) if total_bars else 0.0
        aligned_chain_coverage_pct = round((len(used_chain_ts) / total_chains) * 100.0, 4) if total_chains else 0.0
        reasons = [
        ...
        return RuntimeSnapshotSanityReport(
            ...
            event_linked_snapshot_count=sum(1 for snapshot in prepared_dataset.snapshots if snapshot.next_event_at is not None),
            reasons=reasons,
        )
    ...
    def _prepare_one_snapshot(
        ...
        *,
        ...
        spot_to_pin_distance_pct = self._spot_to_pin_distance_pct(aligned_bar.close, dominant_strike)
        ...
        pin_progression_sequence = [
            PreparedPinProgressionPoint(ts=point.ts, distance_to_pin_pct=point.spot_to_pin_distance_pct)
        ...
        if session_vwap not in {None, 0.0}:
            ...
            distance_to_vwap_pct = round(self._pct_change(float(aligned_bar.close), float(session_vwap)), 4)
        ...
        if prior_close_price not in {None, 0.0} and session_open_price is not None:
            ...
            prior_session_return_pct = round(self._pct_change(float(session_open_price), float(prior_close_price)), 4)
        return PreparedRuntimeSnapshot(
            ...
            opening_range_break_count=self._opening_range_break_count(bars_up_to_ts, opening_range_high, opening_range_low),
            price_realised_vol_5m_pct=round(self._realised_vol_proxy(chain.ts, ordered_bars, lookback_bars=5), 4),
            price_realised_vol_15m_pct=round(self._realised_vol_proxy(chain.ts, ordered_bars, lookback_bars=15), 4),
            relative_volume_ratio=round(self._relative_volume_ratio(bars_up_to_ts, lookback_bars=5), 4),
            ...
            front_realised_vol=round(self._realised_vol_proxy(chain.ts, ordered_bars, lookback_bars=6), 4),
            next_realised_vol=round(self._realised_vol_proxy(chain.ts, ordered_bars, lookback_bars=12), 4),
            snapshot_sequence_id=chain.sequence.sequence_id if chain.sequence is not None else None,
            ...
            snapshot_window_minutes=chain.sequence.window_minutes if chain.sequence is not None else None,
    ...
    def _nearest_call_put(self, quotes: Sequence[OptionQuote], spot_price: float) -> tuple[OptionQuote, OptionQuote]:
    ...
    def _near_spot_oi(self, quotes: Sequence[OptionQuote], spot_price: float, *, side: str) -> float:
        """Return near-spot open interest for one side of the chain."""

        return sum(
            float(quote.oi or 0.0)
            for quote in quotes
            if quote.side == side and abs((float(quote.strike) - spot_price) / max(spot_price, 1.0)) <= 0.02
        )
    ...
    def _dominant_strike(self, quotes: Sequence[OptionQuote]) -> float | None:
        ...
        if not quotes:
            return None
        dominant_quote = max(quotes, key=lambda quote: float(quote.oi or 0.0) + float(quote.volume or 0.0))
    ...
    def _tenor_iv_curve(self, chain: OptionChainSnapshot, spot_price: float) -> list[PreparedTenorPoint]:
    ...
    def _nearby_strike_clusters(self, quotes: Sequence[OptionQuote], spot_price: float) -> list[PreparedStrikeCluster]:
        ...
        for quote in ordered_quotes[:4]:
            clusters.append(
                PreparedStrikeCluster(
                    ...
                    volume=float(quote.volume or 0.0),
                    distance_to_spot_pct=round(abs((float(quote.strike) - spot_price) / max(spot_price, 1.0)) * 100.0, 4),
                )
            )
        return clusters
    ...
    def _next_event(self, ts: datetime, events: Sequence[EventRecord]) -> EventRecord | None:
        ...
        future_events = sorted((event for event in events if event.event_at >= ts), key=lambda event: event.event_at)
    ...
    def _session_vwap(self, bars: Sequence[BarRecord]) -> float | None:
        ...
        if total_volume <= 0.0:
            return None
        weighted_total = sum((((float(bar.high) + float(bar.low) + float(bar.close)) / 3.0) * float(bar.volume)) for bar in bars)
        return weighted_total / total_volume

    def _opening_range_5m(self, ordered_bars: Sequence[BarRecord]) -> tuple[float | None, float | None]:
        ...
        if not same_day:
            return None, None
        return round(max(float(bar.high) for bar in same_day), 4), round(min(float(bar.low) for bar in same_day), 4)
    ...
    def _realised_vol_proxy(self, ts: datetime, ordered_bars: Sequence[BarRecord], *, lookback_bars: int) -> float:
        ...
        if len(closes) < 2:
            return 0.0
        returns = [((right / left) - 1.0) * 100.0 for left, right in zip(closes, closes[1:], strict=False) if left > 0.0]
        if not returns:
            return 0.0
